...
const cleanAr=(text,surahNum,verseNum)=>{
  let t=text;
  // Strip Bismillah from verse 1 of surahs (except Al-Fatihah where it IS verse 1)
  if(verseNum===1&&surahNum!==1&&surahNum!==9){
    // Match any form of Bismillah at the start (various Unicode spellings from different APIs)
    t=t.replace(/^\s*بِسْمِ\s+[ٱا]للَّهِ\s+[ٱا]لرَّحْمَ[ـٰ]*[نٰ]ِ\s+[ٱا]لرَّحِيمِ\s*/,"")
       .replace(/^\s*بِسۡمِ\s+[ٱا]للَّهِ\s+[ٱا]لرَّحۡمَ[ـٰ]*[نٰ]ِ\s+[ٱا]لرَّحِيمِ\s*/,"");
  }
  return t.trim();
};
