...
// EveryAyah CDN — most reliable ayah-by-ayah audio source
// URL format: https://everyayah.com/data/{reciterFolder}/{surah3digits}{ayah3digits}.mp3
const AUDIO_RECITERS={
  "ar.alafasy":"Alafasy_128kbps",
  "ar.abdulbasitmurattal":"Abdul_Basit_Murattal_192kbps",
  "ar.abdurrahmaanas-sudais":"Abdurrahmaan_As-Sudais_192kbps",
...
const audioSrc = (recId, surahNum, ayahNum) => {
  const folder = AUDIO_RECITERS[recId] || "Alafasy_128kbps";
...
// Clean Arabic text: strip Bismillah from verse 1 (API embeds it)
const cleanAr=(text,surahNum,verseNum)=>{
  if(verseNum!==1||surahNum===1||surahNum===9)return text;
  // The API embeds Bismillah at the start of verse 1 for most surahs
  // Detect it by looking for "بسم" at start and "الرحيم" nearby
  const t=text.trim();
  if(!t.startsWith("\u0628\u0650\u0633\u0652\u0645\u0650")&&!t.startsWith("\u0628\u0650\u0633\u0645\u0650")&&!t.startsWith("بِسْمِ")&&!t.startsWith("بِسۡمِ"))return t;
  // Find the position after "الرحيم" or "ٱلرَّحِيمِ"
  const idx1=t.indexOf("\u0627\u0644\u0631\u0651\u064E\u062D\u0650\u064A\u0645\u0650"); // الرَّحِيمِ
  const idx2=t.indexOf("\u0631\u0651\u064E\u062D\u0650\u064A\u0645\u0650"); // رَّحِيمِ
  if(idx1>0){return t.substring(idx1+10).trim()}
  if(idx2>0){return t.substring(idx2+9).trim()}
  // Fallback: try splitting by الرحيم
  const parts=t.split(/الرَّحِيمِ|ٱلرَّحِيمِ/);
...
export default function NoorApp() {
  ...
  // Fetch Mushaf page word data via Vercel proxy (bypasses CORS)
  useEffect(()=>{
    if(readMode!=="mushaf")return;
    (async()=>{
      setMushafLd(true);setMushafLines(null);
      try{
        const r=await fetch(`/api/quran?page=${mushafPage}`);
        const d=await r.json();
        if(d.verses){
          const lineMap={};
          d.verses.forEach(v=>{
  ...
  const playV=(v,cr)=>{if(!audio)return;const r=cr||rec.id;const k=`${surah?.n||0}:${v.num}`;
    if(playing===k){audio.pause();setPlaying(null);return}
    const url=audioSrc(r,surah?.n||1,v.num);
    audio.src=url;
    audio.load();
    const p=audio.play();
    if(p&&p.catch)p.catch(e=>console.log("Audio error:",e,url));
    setPlaying(k);
  };
  ...
  <div style={{fontFamily:"'Manrope',sans-serif",minHeight:"100vh",background:T.bg,color:T.tx,overflowX:"hidden",width:"100%",transition:"background .5s,color .5s"}}>
    ...
    *{margin:0;padding:0;box-sizing:border-box}html,body{overflow-x:hidden}
    ...
    {tab==="read"&&surah&&<div className="fi">
      ...
      {/* Bismillah — shown separately above verses (stripped from verse 1 text) */}
      {readMode!=="mushaf"&&surah.n!==1&&surah.n!==9&&<div style={{textAlign:"center",fontFamily:"'UthmanicHafs','Amiri Quran',serif",fontSize:"clamp(20px,5vw,28px)",color:T.g,padding:"14px 0",marginBottom:12}}>بِسْمِ ٱللَّهِ ٱلرَّحْمَـٰنِ ٱلرَّحِيمِ</div>}
      ...
      {readMode==="mixed"&&vrs.map((v,i)=>{
        ...
        const arText=cleanAr(v.ar,surah.n,v.num);
        return(
          <div key={v.num} style={{marginBottom:6,padding:"14px 14px",background:T.cardBg,
            ...
            <div style={{display:"flex",gap:10,alignItems:"flex-start"}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontFamily:"'UthmanicHafs','Amiri Quran',serif",fontSize:"clamp(18px,4.5vw,24px)",lineHeight:2,
                  direction:"rtl",textAlign:"right",color:T.arabicColor,marginBottom:8,overflowWrap:"anywhere"}}>{arText}</div>
      ...
      {readMode==="study"&&vrs.map((v,i)=>{const k=`${surah.n}:${v.num}`;const isP=playing===k;const isB=bm.includes(k);const tr=trVrs[i]?.text||"";const tr2=trVrs2[i]?.text||"";const hasN=journal[k];const arText=cleanAr(v.ar,surah.n,v.num);const words=arText.split(/\s+/);
      return(<div key={k} style={{marginBottom:9,padding:16,...crd,borderRadius:12,borderColor:isP?DARK.em:T.bd,transition:"all .3s"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
          <div style={{width:30,height:30,display:"flex",alignItems:"center",justifyContent:"center",background:`linear-gradient(135deg,${DARK.ed},${DARK.em})`,color:DARK.gl,borderRadius:"50%",fontSize:11,fontWeight:700}}>{v.num}</div>
        ...
        {hifzMode?<div style={{fontFamily:"'UthmanicHafs','Amiri Quran',serif",fontSize:"clamp(20px,5vw,26px)",lineHeight:2.2,textAlign:"right",direction:"rtl",marginBottom:12,paddingBottom:12,borderBottom:`1px solid ${T.bd}`,display:"flex",flexWrap:"wrap",justifyContent:"flex-end",gap:"3px 8px"}}>
        ...
        <div style={{fontFamily:"'UthmanicHafs','Amiri Quran',serif",fontSize:"clamp(20px,5vw,26px)",fontWeight:400,lineHeight:2.1,textAlign:"right",direction:"rtl",color:T.arabicColor,marginBottom:12,paddingBottom:12,borderBottom:`1px solid ${T.bd}`,wordBreak:"break-word",overflowWrap:"anywhere"}}>{arText}</div>}
    ...
    {tab==="mood"&&<div className="fi">
      {!moodMode?<div>
        ...
        moodVerses.map((mv,i)=><div key={i} style={{...crd,marginBottom:8,padding:16}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><span style={{fontSize:10.5,color:T.g,fontWeight:600}}>{mv.ref}</span><button onClick={()=>{const sn=SU.find(s=>mv.ref.includes(s.e));playV({num:mv.num},MOODS[moodMode].reciter)}} style={{width:28,height:28,borderRadius:"50%",border:"none",background:`linear-gradient(135deg,${DARK.em},${DARK.el})`,color:"white",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}>{IC.play}</button></div>
